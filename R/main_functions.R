...
get_fire_summary <- function(outdir, prefix) {
  ...
  list(
    total = total.area,
    surface = type.areas["surface"],
    passive.crown = type.areas["passive"],
    active.crown = type.areas["active"]
  )
}
...
get_fire_area <- function(outdir, prefix) {
  ...
  r <- raster::raster(file.path(outdir, fname))

  x <- raster::values(r)
  ncells <- sum(!is.na(x))
  cellarea <- prod( raster::res(r) )
